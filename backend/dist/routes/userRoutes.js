"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const userController_1 = require("../controllers/userController");
const auth_1 = require("../middlewares/auth");
const router = (0, express_1.Router)();
router.get('/me', auth_1.authenticateToken, userController_1.getMe);
router.get('/me/notifications', auth_1.authenticateToken, userController_1.getMyNotifications);
router.get('/me/notifications/:notificationId', auth_1.authenticateToken, userController_1.getMyNotification);
router.patch('/me/notifications/:notificationId/read', auth_1.authenticateToken, userController_1.markMyNotificationRead);
router.delete('/me/notifications/:notificationId', auth_1.authenticateToken, userController_1.deleteMyNotification);
router.get('/me/wellbeing', auth_1.authenticateToken, userController_1.getMyWellbeing);
router.put('/me', auth_1.authenticateToken, userController_1.updateMe);
router.get('/', auth_1.authenticateToken, (0, auth_1.requireRole)(['Admin', 'Manager']), userController_1.getAllUsers);
router.get('/roles', auth_1.authenticateToken, (0, auth_1.requireRole)(['Admin', 'Manager']), userController_1.getRoles);
router.get('/:id/auth-events', auth_1.authenticateToken, (0, auth_1.requireRole)(['Admin', 'Manager']), userController_1.getUserAuthEvents);
router.post('/', auth_1.authenticateToken, (0, auth_1.requireRole)(['Admin', 'Manager']), userController_1.createUser);
router.post('/import', auth_1.authenticateToken, (0, auth_1.requireRole)(['Admin', 'Manager']), userController_1.importUsers);
router.put('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['Admin', 'Manager']), userController_1.updateUser);
router.delete('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['Admin', 'Manager']), userController_1.deleteUser);
router.delete('/:id/permanent', auth_1.authenticateToken, (0, auth_1.requireRole)(['Admin']), userController_1.permanentlyDeleteUser);
exports.default = router;
