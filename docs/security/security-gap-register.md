...
| SEC-03 | `/api-docs`, `/uploads`, token share routes are reachable without JWT | Public metadata/file exposure needs intended-access review; **Medium** | Review output and disable/protect only with compatibility plan; Open | Route response/access tests; revert routing/config change |
| SEC-04 | Router authentication and role gates are present | Handler-level organisation/object scope not proven across all identifiers; **High pending evidence** | Trace sensitive controllers and add BOLA/tenant tests; Open | Cross-user/cross-org integration tests; revert focused predicate change |
